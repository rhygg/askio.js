...
const Arrayrunner = require('../core/utils/array');

class Survey extends Arrayrunner {
  constructor(options = {}) {
    super(options);
  ...
  async render() {
    ...
    let runner = [prefix, message, separator].filter(Boolean).join(' ');
    ...
    if (output || !help) runner += ' ' + output;
    if (help && !runner.includes(help)) runner += ' ' + help;

    if (submitted && !output && !body && this.multiple && this.type !== 'form') {
      runner += this.styles.danger(this.emptyError);
    }

    this.clear(size);
    this.write([runner, header, body, footer].filter(Boolean).join('\n'));
    this.restore();
  }
