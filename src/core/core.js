...
const ScreenManager = require('./utils/manager');
const {Color, Beautify} = require('callista')
const defaultState = {
  validate: () => true,
  filter: (val) => val,
  transformer: (val) => val,
...
class StateManager {
  ...
  startLoading() {
    ...
    setTimeout(this.onLoaderTick, Beautify.spinner.interval);
  }
  ...
  getPrefix() {
    ...
    if (status === 'loading') {
      const frame = loadingIncrement % Beautify.spinner.frames.length;
      prefix = Color.yellow(Beautify.spinner.frames[frame]);
    }
