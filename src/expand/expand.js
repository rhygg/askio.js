...
module.exports = createPrompt((config, done) => {
    ...
    if (isEnterKey(key)) {
      ...
      if (answer === 'h' && !expanded) {
        ...
        if (selectedChoice) {
          ...
          setError(`"${Color.red(value)}" isn't an available option`);
        }
      }
    } else {
      setValue(rl.line);
  ...
  if (status === 'done') {
    return `${prefix} ${message} ${Color.cyan(value)}`;
  }
  ...
  let shortChoices = allChoices
    ...
    .join('');
  shortChoices = Color.dim(` (${shortChoices})`);
  ...
  if (currentOption) {
    helpTip = `${Color.cyan('>>')} ${currentOption.name || currentOption.value}`;
  }
